...
def get_activity(orders_path, trades_path, ins_type: str, utc_offset: int) -> tuple:
    ...
    # Individual trades with price, direction, and private flag
...
GRID_COLOR   = "#1c2333"
AXIS_COLOR   = "#8b949e"
COL_BID      = "#26a69a"   # green (bid / buy)
COL_ASK      = "#ef5350"   # red   (ask / sell)
COL_DELIVERY = "#f0e130"

fig = go.Figure()

# ── Bid orders — green step line ─────────────────────────────────────────────
if not bid_df.empty:
...
# ── Public buy trades — small green circles ───────────────────────────────────
if not pub_buy_trades.empty:
    fig.add_trace(go.Scatter(
        ...
        hovertemplate="%{x|%H:%M:%S}  Buy @ %{y:.2f}  Vol: %{customdata:.1f}<extra></extra>",
        customdata=pub_buy_trades["VOLUME"],
    ))

# ── Public sell trades — small red circles ────────────────────────────────────
if not pub_sell_trades.empty:
    fig.add_trace(go.Scatter(
        ...
        hovertemplate="%{x|%H:%M:%S}  Sell @ %{y:.2f}  Vol: %{customdata:.1f}<extra></extra>",
        customdata=pub_sell_trades["VOLUME"],
    ))

# ── Private buy trades — large green diamonds ─────────────────────────────────
if not priv_buy_trades.empty:
    fig.add_trace(go.Scatter(
        x=priv_buy_trades["ts_cet"], y=priv_buy_trades["PRICE"],
        name="🔒 Private Buy",
        mode="markers",
        marker=dict(color=COL_BID, size=13, symbol="diamond",
                    line=dict(color="#ffffff", width=1.5)),
        hovertemplate="%{x|%H:%M:%S}  🔒 Buy @ %{y:.2f}  Vol: %{customdata:.1f}<extra></extra>",
        customdata=priv_buy_trades["VOLUME"],
    ))

# ── Private sell trades — large red diamonds ──────────────────────────────────
if not priv_sell_trades.empty:
    fig.add_trace(go.Scatter(
        x=priv_sell_trades["ts_cet"], y=priv_sell_trades["PRICE"],
        name="🔒 Private Sell",
        mode="markers",
        marker=dict(color=COL_ASK, size=13, symbol="diamond",
                    line=dict(color="#ffffff", width=1.5)),
        hovertemplate="%{x|%H:%M:%S}  🔒 Sell @ %{y:.2f}  Vol: %{customdata:.1f}<extra></extra>",
        customdata=priv_sell_trades["VOLUME"],
    ))
...
if _tc17_res and _tc17_res.get("data") is not None:
    ...
    if not _raw.empty and "INS_TYPE" in _raw.columns:
        # Keep only bid-side rows for this INS_TYPE
        _inv_df = _raw[
            (_raw["INS_TYPE"] == sel_ins) & (_raw["BID_ASK"] == "B")
        ].copy()
        if not _inv_df.empty:
            # Convert TRAN_DATETIME to CET
